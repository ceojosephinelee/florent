...
### [DEBT-037] 회원 탈퇴 시 영향받은 상대방 알림 미발송

- **유형**: 기술 부채 (기능 제한)
- **위치**: `application/auth/WithdrawService.java`
- **내용**: 판매자 탈퇴 시 해당 판매자의 OPEN 상태 제안서가 삭제되지만, 영향받은 구매자에게 알림이 발송되지 않음. 구매자 탈퇴 시에도 해당 요청에 제안을 작성 중인 판매자에게 알림 미발송.
- **심각도**: Medium
- **해결 조건**: 실사용자 유입 후 탈퇴 시나리오에서 혼란 발생 시
- **상태**: OPEN

---

### [DEBT-038] 회원 탈퇴 시 카카오 연동 해제 미구현

- **유형**: 기술 부채 (기능 제한)
- **위치**: `application/auth/WithdrawService.java`
- **내용**: 탈퇴 시 카카오 OAuth 연결 끊기(unlink) API 호출이 없음. 재가입 시 카카오 계정 연결이 남아 있을 수 있음. MVP 이후 KakaoOAuthPort에 unlink 메서드 추가 필요.
- **심각도**: Low
- **상태**: OPEN
