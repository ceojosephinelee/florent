...
Feature: 구매자 제안 선택 및 예약 확정
  구매자는 SUBMITTED 상태의 제안을 선택하여 PENDING_CONTACT 상태의 예약을 생성할 수 있다.
  ...
  Scenario: 구매자가 SUBMITTED 제안을 선택하면 예약이 PENDING_CONTACT 상태로 생성된다
  ...
  Scenario: 이미 예약된 요청에 같은 제안을 다시 선택하면 422가 반환된다
    Given 구매자가 해당 제안을 선택했다
